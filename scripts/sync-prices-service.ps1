...
#   nssm set HomesaversPricesSync AppStdout C:\Homesavers\logs\sync-prices-service-stdout.log
#   nssm set HomesaversPricesSync AppStderr C:\Homesavers\logs\sync-prices-service-stderr.log
#   nssm start HomesaversPricesSync

param(
  [string]$BaseUrl       = "https://homesaversscanner.pages.dev",
  [string]$SecretFile    = "C:\Homesavers\.sync-secret",
  [string]$LogPath       = "C:\Homesavers\logs\sync-prices-service.log",
  [string]$SyncScript    = "$PSScriptRoot\sync-prices.ps1",
...
$script:namePrefix     = 'ItemMaster'
$script:sheet          = 'ItemMaster'
...
function Refresh-Config {
  try {
    ...
    $newPrefix  = if ($cfg.name_prefix)  { $cfg.name_prefix }  else { 'ItemMaster' }
    $newSheet   = if ($cfg.sheet)        { $cfg.sheet }        else { 'ItemMaster' }
    $newTime    = $cfg.time
...
function Attach-Watcher {
  ...
  Register-ObjectEvent -InputObject $w -EventName Created -Action $action -SourceIdentifier "HS_FSW_PR_Created" | Out-Null
  Register-ObjectEvent -InputObject $w -EventName Changed -Action $action -SourceIdentifier "HS_FSW_PR_Changed" | Out-Null
  Register-ObjectEvent -InputObject $w -EventName Renamed -Action $action -SourceIdentifier "HS_FSW_PR_Renamed" | Out-Null
