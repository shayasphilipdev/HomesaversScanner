...
| **7** | **Offline queue + PWA** | ✅ done | `client/public/sw.js` (stale-while-revalidate for SPA shell, cache-first for hashed assets, never intercept /api/*). `client/public/manifest.webmanifest`. IDB outbox at `client/src/lib/outbox.js` queues task records (and Task B's two photo Blobs) when the network fails. **Sync triggers (event-driven, no polling):** `online` event · `visibilitychange` → visible · tap the offline pill (manual). **Bounded retries** — server-side failures count toward attempts (max 5), then the record is marked `failed` and skipped by auto-sync; tap the pill to reset failed and retry. Pure network errors never count against attempts. `OfflineIndicator` pill in the nav shows offline / pending / failed states. |
